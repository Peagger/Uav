...
"""抵达边界所需参数"""
step_len = 0.6  # 移动步长
Detect_Error = 0.01  # 允许误差

"""边界目标位置调整所需参数"""
max_n = (n - m) / m if ((n - m) / m) % 1 == 0 else int((n - m) / m) + 1#每条边上最多几个点
unit_gap = 2 * r * np.sin(np.pi / m) / (max_n + 1)  # 2 * R * np.sin(np.pi/n)是边界长度， unit_gap是调整单位距离

"""均匀化参数"""
Stop_Gap_NUM = 3  # 齿链上，与目标节点仅仅相小于3个节点，则判断为编队完成
Move_Gap_Num = 50  # 20 * 0.01 =0.2 齿链上，步长转换成节点数

"""随机生成无人机初始位置,记录为Point_List"""
pxs = np.random.randint(10, 110, n)
pys = np.random.randint(10, 110, n)
pzs = [0 for i in range(n)]
Point_List = [[pxs[i], pys[i], 0] for i in range(n)]

""" 计算多边形顶点位置"""
Vertexs_List = [[cx + r * np.sin(i * 2 * np.pi / m), cy + r * np.cos(i * 2 * np.pi / m), cz] for i in range(1, m + 1)]
...
verzs.append(Vertexs_List[0][2])#首尾相连
...
num = int(d / 0.01)#每个边上1/0.01个节点
tooth_distance = d / num
Vertexs_Tooth_Chain = []#每个节点的坐标
...
for i in range(0, m):
    ...
    angle = np.arccos(cos_angle)#指向下一个顶点的向量与x轴的角度
    if x[0] >= 0 and x[1] >= 0:
...
class Point_1():
    ...
    def decide0(self):
        ...
        if abs(cz - my_pos[2]) < hv:
            ...
            aim = my_pos[2] + hv#aim为类内局部变量，疑似可以删除
    ...
    def occupy(self, nearest):
        """检测是谁占领的该多边形顶点，返回其id"""#距离小于Detect_Error视为占领
    ...
    def adjust(self, idx):
        ...
        for each in Vertexs_List:
            ...
            if d <= Detect_Error:
                identity = Vertexs_List.index(each)#被占领点id
...
class Point2():
    ...
    next_dis = len(Vertexs_Tooth_Chain) + 1 #间隔距离
...
def gen():
    ...
    while True:
        ...
        if state_flag == 0:
            ...
            for i in range(n):
                Obj_List[i].decide2(Vertexs_List)
        elif state_flag == 3:
            ...
            if init_flag == 0:
                ...
                for i in Point_List:
                    for j in Vertexs_Tooth_Chain:
                        if distance_calculate_2D(i, j) <= tooth_distance :
                            li = copy.deepcopy(i)
            ...
            time.sleep(5)
            exit()
        if Point_List2 == Point_List:#没有变化状态+1
            state_flag += 1
        else:
            pass
        if state_flag>1:
...
def move():
    ...
    global Vertexs_Tooth_Chain
    speed=0.3
    # for i in Vertexs_List:
    #     i[0]+=speed
    for i in Vertexs_Tooth_Chain:
        i[0]+=speed
    for i in range(n):
        # if Obj_List[i].arrive==True:
            Point_List[i][0]+=speed
    for i in range(n):
        if Obj_List[i].aim_adjusted!=None:
            Obj_List[i].aim_adjusted[0]+=speed
    

ani = animation.FuncAnimation(fig, update, frames=gen, interval=0.5)
plt.show()
